...
let google_translation = async function (data, fromLanguage, toLanguage, is_need_proxy, proxy_url) {
    ...
    await rp(google_request_option_str).then(function (body) {
        if (body != null) {
            ...
            data.translate_title = final_title_str.replace(/\"/g, "");
            tmpPost.translate_title = final_title_str.replace(/\"/g, "");
            let postStr = front.stringify(tmpPost);
            postStr = '---\n' + postStr;
            fs.writeFileSync(data.full_source, postStr, 'utf-8');
            console.log("Google->Generate link %s for post [%s]", final_title_str, data.title);
            return data;
        }
    }).catch(function (err) {
        console.error(err);
...
let youdao_translation = async function (data, youdao_api_key, youdao_keyfrom) {
    ...
    await rp(youdaoTransUrl).then(function (body) {
        if (body != null) {
            ...
            postStr = '---\n' + postStr;
            fs.writeFileSync(data.full_source, postStr, 'utf-8');
            console.log("Youdao->Generate link %s for post [%s]", final_title_str, data.title);
            return data;
        }
    }).catch(function (err) {
        console.error(err);
...
let baidu_translation_with_appid = async function (data,fromLanguage,toLanguage,appid,appkey){
    ...
    await rp(baiduTransOption).then(function (body) {
        if (body != null) {
            ...
            postStr = '---\n' + postStr;
            fs.writeFileSync(data.full_source, postStr, 'utf-8');
            //console.log("Baidu->Generate link %s for post [%s]", final_title_str, data.title);
            return data;
        }
    }).catch(function (err) {
        console.error(err);
...
let baidu_translation_no_appid = async function (data, fromLanguage, toLanguage) {
    ...
    await rp(token_options).then(function (response) {
        if (response && response.statusCode == 200) {
            ...
            // let cookies = response.headers['set-cookie'].map(Cookie.parse);
            // console.log('cookies==>',cookies);
            // cookiejar.setCookie(cookies,baiduFanyiUrl);
        }
    }).catch(function (err) {
    ...
    let postHeaders = {
        ...
        'Cookie':'locale=zh; BAIDUID='+final_baiduid+' from_lang_often=%5B%7B%22value%22%3A%22en%22%2C%22text%22%3A%22%u82F1%u8BED%22%7D%2C%7B%22value%22%3A%22zh%22%2C%22text%22%3A%22%u4E2D%u6587%22%7D%5D; REALTIME_TRANS_SWITCH=1; FANYI_WORD_SWITCH=1; HISTORY_SWITCH=1; SOUND_SPD_SWITCH=1; SOUND_PREFER_SWITCH=1; to_lang_often=%5B%7B%22value%22%3A%22zh%22%2C%22text%22%3A%22%u4E2D%u6587%22%7D%2C%7B%22value%22%3A%22en%22%2C%22text%22%3A%22%u82F1%u8BED%22%7D%5D;',
    ...
    // return false;
    await rp(options).then(function (error, response, body) {
    ...
    //TODO:上面的返回的总是998，Cookie超时，应该是计算sign的时候有问题
};

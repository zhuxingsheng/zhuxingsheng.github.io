...
let baidu_translation = function(data,fromLanguage,toLanguage){
  ...
  var appid = '20171231000110878';
	var key = 'qzbvtY13TbZYiJjRaJuj';
	var salt = (new Date).getTime();
	var str1 = appid + title + salt +key;
  var sign = MD5(str1);
  let baiduTransUrl = "http://api.fanyi.baidu.com/api/trans/vip/translate?sign="+sign+"&appid="+appid+"&salt="+salt+"&from="+fromLanguage+"&q="+encodedStr+"&to="+toLanguage;
  ...
  request.get({
              ...
              if(response && response.statusCode==200){
				  console.log(body);
                  let json_str = JSON.parse(body);
				  console.log(json_str.trans_result[0].dst);
                  let title_array = json_str.trans_result[0].dst.replace(/\s/g,"-").replace(/(.)\1+/gi,'$1').split("-");
                  let tmp_title_array = [];
                  let final_title_str = '';
                  for(var i=0;i<title_array.length;i++)
                  {
